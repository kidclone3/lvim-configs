...
if (not vim.g.vscode) 
then
keymap("n", "<F9>", ':w <cr>:AsyncRun g++-12 -std=c++17 "$(VIM_FILEPATH)" -o "$(VIM_FILEDIR)/a" -Wshadow -Wall -O2 -Wno-unused-result <cr>', opts)
keymap("i", "<F9>", '<ESC> :w <cr>:AsyncRun g++-12 -std=c++17 "$(VIM_FILEPATH)" -o "$(VIM_FILEDIR)/a" -Wshadow -Wall -O2 -Wno-unused-result <cr>', opts)
